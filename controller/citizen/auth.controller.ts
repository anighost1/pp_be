...
export const login = async (req: Request, res: Response) => {
    try {
        const defaultOtp = 697869
        const { contact_no, otp } = req.body;

        if (Number(otp) !== defaultOtp) {
            throw new Error('Invalid OTP')
        }

        let user: any

        user = await panel.user.findFirst({
            where: {
                phone: contact_no
            },
            include: {
        ...
        if (!user) {
            ...
            const defaultPassword = "testing";
            const hashedPassword = await bcrypt.hash(defaultPassword, 10);

            user = await panel.user.create({
                data: {
                    username: String(userChargeData?.owner_name?.slice(0, 4)),
                    phone: String(contact_no),
        ...
        if (isSuperAdmin) {
            ...
            permissions = new Set<any>(
                user.roles.flatMap((role: any) => role.permissions.map((p: any) => ({ id: p.id, name: p.name, from: 'role' }))),
            );
            user.permissions.forEach((p: any) => permissions.add({ id: p.id, name: p.name, from: 'permission' }));
            // user.revokedPermissions.forEach(p => permissions.delete(p.id));
            user.revokedPermissions.forEach((p: any) => deleteById(permissions, p.id));
            ...
            user.roles.forEach((role: any) => {
                role.permissions.forEach((permission: any) => {
                    permission.menus.forEach((menu: any) => {
                        menuMap.set(menu.id, {
                            id: menu.id,
                            label: menu.label,
                            path: menu.path,
            ...
            user.revokedPermissions.forEach((permission: any) => {
                permission.menus.forEach((menu: any) => {
        ...
        const payload = {
            ...
            ulb: user?.ulb.map((ulb: any) => ({ id: ulb?.id, name: ulb?.name })),
            ward: user?.ward.map((w: any) => ({
                id: w.id,
                ward_no: w.ward_no
            })),
            isSuperAdmin,
